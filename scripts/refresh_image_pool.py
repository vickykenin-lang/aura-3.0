...
def main() -> int:
    ...
    for search in config.get("search_terms", []):
        ...
        for _ in range(max_pages_per_search):
            ...
            if not consumed_entire_page:
                # Revisit this page next run; seen keys prevent duplicates while
                # allowing unprocessed candidates on the same page to be found.
            ...
            if has_more:
                ...
                # Wrap after reaching the end. Seen-image history prevents reuse
                # while allowing future newly indexed Commons results to surface.
    ...
    # Persist scan progress even when no image is acquired. This is what allows
    # the next autonomous/hourly run to continue beyond previously scanned pages.
