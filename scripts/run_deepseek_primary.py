...
def deepseek_generate(_unused_key: str, selected: list[dict]) -> tuple[list[dict], str]:
    ...
    count = len(selected)
    inputs = [{"slot": i + 1, "room_tag": item.get("photo_tag", "interior"), "content_angle": item.get("angle", "")} for i, item in enumerate(selected)]
    prompt = generator.SYSTEM_PROMPT + (f"\n\nCreate exactly {count} items for these fixed image slots:\n" + json.dumps(inputs, ensure_ascii=False) + '\nReturn JSON only as {"candidates":[{"slot":1,"hook_en":"...","caption_hi":"...","hashtags":"#... #..."}]}. ' + f"The candidates array must contain exactly {count} items.")
    ...
    request = urllib.request.Request(DEEPSEEK_API, data=json.dumps(payload).encode("utf-8"), headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, method="POST")
...
def governed_metadata_visual_gate(_unused_key: str, image_url: str) -> dict:
    ...
    return {"visual_ok": True, "room_type": "other", "quality": 7, "reasons": ["governed curated image-pool metadata gate; no vision-model inspection claimed"], "model": "CURATED_METADATA_GATE"}
