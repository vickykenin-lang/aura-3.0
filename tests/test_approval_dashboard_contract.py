...
class ApprovalDashboardContractTests(unittest.TestCase):
    ...
    def test_rejection_refills_but_approval_does_not(self):
        text = (ROOT / '.github/workflows/aura3-founder-approval.yml').read_text(encoding='utf-8')
        self.assertIn("steps.command.outputs.action == 'reject'", text)
        self.assertIn("'trigger_reason': 'founder_rejection'", text)
        self.assertNotIn("'trigger_reason': 'founder_approval'", text)
